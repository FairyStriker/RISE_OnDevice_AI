...
class DeepStreamPipeline:
    ...
    def _create_pipeline(self):
        ...
        # nvv4l2h264enc(GPU/Hardware) 사용
        encoder = Gst.ElementFactory.make("nvv4l2h264enc", "h264-encoder")
        encoder.set_property("bitrate", 4000000) # 4Mbps (AGX는 대역폭이 넉넉함)
        encoder.set_property("preset-level", 1)  # UltraFast High Performance
        encoder.set_property("insert-sps-pps", 1)
        #encoder.set_property("bufapi-version", 1)

        rtppay = Gst.ElementFactory.make("rtph264pay", "rtppay")
        
        # 안전장치용 Leaky Queue 생성
        queue_safety = Gst.ElementFactory.make("queue", "queue_safety")
        queue_safety.set_property("leaky", 2)             # 큐가 차면 오래된 데이터 버림
        queue_safety.set_property("max-size-buffers", 10)  # 큐에 1프레임만 유지 (지연 방지)
        ...
        elements = [source, streammux, pgie, tracker, nvvidconv1, nvosd, 
                    nvvidconv2, encoder, rtppay, queue_safety, udpsink]
    ...
    def bus_call(self, bus, message, loop):
        ...
        if t == Gst.MessageType.EOS: loop.quit()
            ...
            print(f"⚠️ [Stream Error] {err}-{debug}")
            loop.quit()
        return True
